...
- **`copy-auditor`** — a Senior Staff Copywriter that reads the docs when present as ground truth (removed from project), audits all site copy for factual mismatches against the protocol, and rewrites to the institutional register (eliminating startup vocabulary, hedging, and passive construction). Copy-only — no browser tools.
