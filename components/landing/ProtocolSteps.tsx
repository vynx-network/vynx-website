...
const steps: Step[] = [
  {
    number: "01",
    title: "AGENT SIGNS",
    body: "EIP-712 payload. Relayer validates: InputToken = USDC, SHF ≥ 1.20, Exposure < 80%. Private mempool.",
  },
  {
    number: "02",
    title: "SEALED BID",
  ...
  },
  {
    ...
    body: (
      <>
        Solver executes <span className={mono}>lockIntent()</span> on{" "}
        <span className={mono}>VynxSettlement.sol</span>. Agent capital locked
        on Base before destination pay.
      </>
    ),
  },
  {
    ...
    <>
      ...
      <div className="mt-12 bg-vynx-bg-card border border-[var(--color-border)] rounded-[2px] py-5 px-6">
        <p className="font-body font-medium text-[16px] text-white border-l-2 border-vynx-gold pl-6">
          &ldquo;Money does not move until the math is irrefutable.&rdquo;
        </p>
      </div>
    </>
  );
